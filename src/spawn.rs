...
use crate::art;
use crate::entity::{Entity, EntityType, Frame, OnDeath, rand_color, resolve_fish_mask};
...
const Z_WATER_GAP3: i32 = 3; // ducks, dolphins, swan, submarine
const Z_WATER_LINE1: i32 = 6; // fishing line and hook
...
/// `rng.random_range(lo..hi)`, but safe when the range is empty on tiny terminals.
...
pub fn seaweed(w: u16, h: u16, tick: u64, rng: &mut impl Rng) -> Entity {
    ...
    let speed = rng.random_range(0.25..0.30); // rand(.05) + .25
...
pub fn fish(w: u16, h: u16, classic: bool, rng: &mut impl Rng) -> Entity {
    let new = !classic && rng.random_range(0..12) > 8; // ~25%
    let arr: &[&str] = if new { &art::NEW_FISH } else { &art::OLD_FISH };
    let count = arr.len() / 2;
    let fish_num = rng.random_range(0..count);
    let idx = fish_num * 2;
    ...
    let mut speed = rng.random_range(0.25..2.25);
    if !right {
        speed = -speed;
    }
...
pub fn random_object(w: u16, h: u16, classic: bool, rng: &mut impl Rng) -> Vec<Entity> {
    match rng.random_range(0..11) {
        ...
        4 => vec![sword_fish(w, h, rng)],
        5 => shark(w, h, rng),
        6 => vec![submarine(w, rng)],
        7 => fishhook(w, h, rng),
        8 => vec![swan(w, rng)],
        9 => vec![ducks(w, rng)],
        _ => dolphins(w, rng),
...
pub fn big_fish(w: u16, h: u16, classic: bool, rng: &mut impl Rng) -> Entity {
    let second = !classic && rng.random_range(0..3) > 1; // ~1/3
...
/// The original long-billed swordfish.
pub fn sword_fish(w: u16, h: u16, rng: &mut impl Rng) -> Entity {
    let dir = rng.random_range(0..2);
    let (x, speed) = if dir == 1 {
        ((w as i32 - 1) as f64, -3.5)
    } else {
        (-33.0, 3.5)
    };
    let y = between(rng, 9, h as i32 - 14) as f64;
    let mask = rand_color(art::SWORD_FISH[2 + dir], rng);
    let mut e = ent(
        EntityType::SwordFish,
        x,
        y,
        Z_SHARK,
        vec![Frame::new(art::SWORD_FISH[dir], &mask)],
    );
    e.dx = speed;
    e.default_color = Color::Yellow;
    e.auto_trans = true;
...
pub fn ducks(w: u16, rng: &mut impl Rng) -> Entity {
    let dir = rng.random_range(0..2);
    let (x, speed, base) = if dir == 1 {
        ((w as i32 - 2) as f64, -1.0, 3)
    } else {
        (-30.0, 1.0, 0)
    };
    let frames = (0..3)
        .map(|i| Frame::new(art::DUCKS[base + i], art::DUCKS[6 + dir]))
        .collect();
    let mut e = ent(EntityType::Duck, x, 5.0, Z_WATER_GAP3, frames);
    e.dx = speed;
    e.frame_speed = 0.25;
    e.default_color = Color::White;
    e.auto_trans = true;
...
pub fn swan(w: u16, rng: &mut impl Rng) -> Entity {
    let dir = rng.random_range(0..2);
    let (x, speed) = if dir == 1 {
        ((w as i32 - 2) as f64, -1.0)
    } else {
        (-10.0, 1.0)
    };
    let mut e = ent(
        ...
        1.0,
        Z_WATER_GAP3,
        vec![Frame::new(art::SWAN[dir], art::SWAN[2 + dir])],
    );
    e.dx = speed;
    e.default_color = Color::White;
    e.auto_trans = true;
...
pub fn dolphins(w: u16, rng: &mut impl Rng) -> Vec<Entity> {
    let dir = rng.random_range(0..2);
    let (x, speed, distance, base) = if dir == 1 {
        ((w as i32 - 2) as f64, -1.0, -15.0, 2)
    } else {
        (-13.0, 1.0, 15.0, 0)
    };
    let starts = [
        (x, 5.0, 24),
        (x - distance, 2.0, 12),
        (x - distance * 2.0, 8.0, 0),
    ];
    starts
        .into_iter()
        .enumerate()
        .map(|(index, (dx, y, delay))| {
            let frames = vec![
                Frame::new(art::DOLPHINS[base], art::DOLPHINS[4 + dir]),
                Frame::new(art::DOLPHINS[base + 1], art::DOLPHINS[4 + dir]),
            ];
            let mut e = ent(EntityType::Dolphin, dx, y, Z_WATER_GAP3, frames);
            e.dx = speed;
            e.frame_speed = 0.5;
            e.motion_delay = delay;
            e.default_color = if index == 1 { Color::Blue } else { Color::Cyan };
            e.auto_trans = true;
            // Only the leader checks the screen edge initially. Once it
            // leaves, the main loop enables the followers, just as the Perl
            // group callback does.
            e.die_offscreen = index == 0;
            if index == 2 {
                e.on_death = OnDeath::RandomObject;
            }
            e
...
pub fn submarine(w: u16, rng: &mut impl Rng) -> Entity {
    let dir = rng.random_range(0..2);
    let (x, speed, base) = if dir == 1 {
        ((w as i32 - 2) as f64, -1.0, 9)
    } else {
        (-40.0, 1.0, 0)
    };
    let frames = (0..9)
        .map(|i| Frame::new(art::SUBMARINE[base + i], art::SUBMARINE[18 + dir]))
        .collect();
    let mut e = ent(EntityType::Submarine, x, 6.0, Z_WATER_GAP3, frames);
    e.dx = speed;
    e.default_color = Color::Yellow;
    e.auto_trans = true;
    e.die_offscreen = true;
...
pub fn fishhook(w: u16, _h: u16, rng: &mut impl Rng) -> Vec<Entity> {
    let x = between(rng, 10, w as i32 - 10);
    let y = -4.0;
    let line_shape = format!("{}{}", "|\n".repeat(50), " \n".repeat(6));
    ...
    let mut line = ent(
        ...
        y - 50.0,
        Z_WATER_LINE1,
        vec![Frame::new(&line_shape, "")],
    );
    line.auto_trans = true;
    ...
    let mut hook = ent(
        ...
        y,
        Z_WATER_LINE1,
        vec![Frame::new(art::FISHHOOK[0], "")],
    );
    hook.default_color = Color::Green;
    hook.auto_trans = true;
    hook.die_offscreen = true;
    hook.on_death = OnDeath::RandomObject;

    let mut point = ent(
        EntityType::HookPoint,
        (x + 1) as f64,
        y + 2.0,
        Z_TEETH,
        vec![Frame::new(art::FISHHOOK[1], "")],
    );
