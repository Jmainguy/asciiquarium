...
pub struct Entity {
    ...
    pub age: u32,
    /// Delays path motion for grouped creatures such as the dolphins.
    pub motion_delay: u32,
    /// Set when a hook catches a fish; the complete rig and fish then rise.
    pub retracting: bool,
    pub alive: bool,
}
...
impl Entity {
    ...
    pub fn update(&mut self, tick: u64, screen_w: u16, screen_h: u16) {
        if self.etype == EntityType::Dolphin {
            if self.age < self.motion_delay {
                self.dy = 0.0;
            } else {
                let path_tick = (self.age - self.motion_delay) % 36;
                self.dy = match path_tick {
                    0..=13 => -0.5,
                    16..=29 => 0.5,
                    _ => 0.0,
                };
        ...
        if matches!(
            self.etype,
            EntityType::FishLine | EntityType::FishHook | EntityType::HookPoint
        ) {
            self.dy = if self.retracting {
                -1.0
            } else if self.y + (self.current().height() as f64) < screen_h as f64 * 0.75 {
                1.0
            } else {
                0.0
            };
        ...
        if self.etype == EntityType::Submarine {
            let center = screen_w as f64 / 2.0 - 20.0;
            let crossing_center = (self.dx > 0.0 && self.x < center && self.x + self.dx > center)
                || (self.dx < 0.0 && self.x > center && self.x + self.dx < center);
            if crossing_center {
                self.x = center;
            }
            if (self.x - center).abs() < f64::EPSILON && self.frame < 8.0 {
                self.frame += 0.1;
                self.age += 1;
                return;
            }
    ...
    mask.chars()
        .map(|c| {
            if c.is_ascii_digit() && c != '0' {
                ...
                *map.entry(c)
                    .or_insert_with(|| PALETTE[rng.random_range(0..PALETTE.len() - 1)])
            } else {
                c
            }
        })
